...
public class PreParamCheckProcessor implements Processor {
    ...
    public void process(ProcessContext processContext) {
        ...
        if (Objects.isNull(messageTemplateId) || CollUtil.isEmpty(messageParamList)){
            processContext.setBreakFlag(true).setResult(new BaseResultVO<>(RespStatus.PRE_PARAM_PROBLEM,"模板ID或参数列表为空"));
        }
        ...
        if (CollUtil.isEmpty(resultMessageParamList)) {
            processContext.setBreakFlag(true).setResult(BaseResultVO.fail(RespStatus.PRE_PARAM_PROBLEM, "含接受者的参数列表为空"));
            return;
        }
        ...
        if (resultMessageParamList.stream().anyMatch(messageParam -> messageParam.getReceiver().split(StrPool.COMMA).length > BasicConstant.MAX_RECEIVER_NUM)) {
            processContext.setBreakFlag(true).setResult(BaseResultVO.fail(RespStatus.PRE_PARAM_PROBLEM,"接收者数量超出范围"));
            return;
        }
